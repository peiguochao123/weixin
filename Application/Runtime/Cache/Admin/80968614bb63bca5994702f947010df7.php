    ...
    <script type="text/javascript" src="/Test/library/jquery/jquery-1.4.js"></script>
     <script>         
      function del(lx_Id)
      {
    	  ...
    	  window.location="/Test/index.php/Admin/ModLianxi/del/lx_Id/"+lx_Id;
      }
      
	  function up(lx_Id)
	  {
    	  
    	  window.location="/Test/index.php/Admin/UpLianxi/up/lx_Id/"+lx_Id;
      }
...
<div class="menuDiv">
  ...
  <div id="menuDiv5"><a href="<?php echo U('Admin//Index/index');?>">返回首页</a></div>
       ...
       <!-- 当前位置 -->
    <div class="locationDiv">&nbsp;: 后台管理 : 修改联系信息</div>
    <!-- 正文内容 -->
  ...
  hello();
      ...
      <div class="rightDiv">
        ...
        <table border="1" align="center" width="700" cellpadding="0" cellspacing="0">
          ...
          <?php if(is_array($contact)): $i = 0; $__LIST__ = $contact;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr height="25">
            <td><?php echo ($v["lx_name"]); ?></td>
            <td><?php echo ($v["lx_phone"]); ?></td>
            <td><?php echo ($v["lx_email"]); ?></td>
            <td><?php echo ($v["lx_qq"]); ?></td>
            <td><?php echo ($v["lx_address"]); ?></td>
            
            <td>
            <a href="<?php echo U('Admin/UpLianxi/index');?>" onclick='up(<?php echo ($v["lx_id"]); ?>)'>修改</a>
            <a href="#" onclick='del(<?php echo ($v["lx_id"]); ?>)'>删除</a>
            </td>
